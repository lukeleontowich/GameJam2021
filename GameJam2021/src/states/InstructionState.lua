...
function InstructionState:render()
    ...
    theta = 3 * math.pi / 2.0
    x = x - 16
    love.graphics.draw(gTextures['button_base'], 40, VIRTUAL_HEIGHT / 2, theta, 1, 1)
    love.graphics.setColor(1, 0.1, 0.1, 1)
    love.graphics.draw(gTextures['pressure_button'], 33, VIRTUAL_HEIGHT / 2 - 1, theta, 1, 1)
    love.graphics.setColor(0, 0, 0, 1.0)

    love.graphics.printf('These buttons only activate when', 1, VIRTUAL_HEIGHT / 2 - 15, VIRTUAL_WIDTH, 'center')
    love.graphics.printf('hit with a certain velocity.', 1, VIRTUAL_HEIGHT / 2 - 10, VIRTUAL_WIDTH, 'center')

    love.graphics.setColor(1, 1, 1, 1)

    love.graphics.draw(gTextures['chest'], 33, VIRTUAL_HEIGHT / 2 + 10)

    love.graphics.setColor(0, 0, 0, 1.0)
    love.graphics.printf('The chest only opens when the', 1, VIRTUAL_HEIGHT / 2 + 10, VIRTUAL_WIDTH, 'center')
    love.graphics.printf('blob opening it holds the key.', 1, VIRTUAL_HEIGHT / 2 + 15, VIRTUAL_WIDTH, 'center')
    love.graphics.setColor(1, 1, 1, 1)

    love.graphics.draw(gTextures['portal_tile'], 33, VIRTUAL_HEIGHT / 2 + 35)
    love.graphics.setColor(0, 0, 0, 1.0)
    love.graphics.printf('The portals can move objects and', 1, VIRTUAL_HEIGHT / 2 + 35, VIRTUAL_WIDTH, 'center')
    love.graphics.printf('enemies to the other side.', 1, VIRTUAL_HEIGHT / 2 + 40, VIRTUAL_WIDTH, 'center')
